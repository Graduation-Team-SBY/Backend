...
describe(`GET /workers/profile/reviews`, () => {
  describe(`Success`, () => {
    test(`Success Get Worker's reviews 200`, async () => {
      const response = await request(app).get(`/workers/profile/reviews`).set(`Authorization`, `Bearer ${tokenWorker}`);
...
describe(`GET /profile/histories`, () => {
  describe(`Success`, () => {
    test(`Success Get Client transaction histories 200`, async () => {
      const response = await request(app).get(`/profile/histories`).set(`Authorization`, `Bearer ${tokenClient}`);
    ...
    test(`Success Get Client transaction histories filtered by week 200`, async () => {
      const response = await request(app).get(`/profile/histories?sort=desc&filter=week`).set(`Authorization`, `Bearer ${tokenClient}`);
    ...
    test(`Success Get Client transaction histories by month 200`, async () => {
      const response = await request(app).get(`/profile/histories?filter=year`).set(`Authorization`, `Bearer ${tokenClient}`);
  ...
  describe(`Failed`, () => {
    ...
    test(`Failed 401, Unauthenticated Invalid Token`, async () => {
      const response = await request(app).get(`/profile/histories`).set(`Authorization`, `Bearer ${tokenClient}fwfbda`);
