...
describe(`PATCH /clients/jobs/:jobId/:workerId`, () => {
  ...
  describe(`Failed`, () => {
    test(`Failed 400, Already Picked Worker For The Job`, async () => {
      ...
      const response = await request(app).patch(`/clients/jobs/${jobId}/${workerIdError}`).set(`Authorization`, `Bearer ${tokenClient}`);
    ...
    test(`Failed 401, Unauthenticated Invalid Token`, async () => {
      const jobId = String(jobSeed[2]._id);
      const workerId = String(userSeed[2]._id);
      const response = await request(app).get(`/clients/jobs/${jobId}/${workerId}`).set(`Authorization`, `Bearer ${tokenClient}fwfbda`);
    ...
    test(`Failed 404, Job Not Found`, async () => {
      ...
      const workerId = String(userSeed[2]._id);
      const response = await request(app).patch(`/clients/jobs/${jobIdError}/${workerId}`).set(`Authorization`, `Bearer ${tokenClient}`);
    ...
    test(`Failed 403, Other Client Attempt to pick a worker`, async () => {
      const jobId = String(jobSeed[2]._id);
      const workerId = String(userSeed[2]._id);
      const response = await request(app).patch(`/clients/jobs/${jobId}/${workerId}`).set(`Authorization`, `Bearer ${tokenClientEmpty}`);
...
describe(`GET /workers/jobs`, () => {
  describe(`Success`, () => {
    test(`Success Get Active Job List 200`, async () => {
      const response = await request(app).get(`/workers/job`).set(`Authorization`, `Bearer ${tokenWorker}`);
  ...
  describe(`Failed`, () => {
    test(`Failed 401, Unauthenticated No Token`, async () => {
      const response = await request(app).get(`/workers/job`);
    ...
    test(`Failed 401, Unauthenticated Invalid Token`, async () => {
      const response = await request(app).get(`/workers/job`).set(`Authorization`, `Bearer ${tokenWorker}fwfbda`);
    ...
    test(`Failed 403, Client User attempting to worker routes`, async () => {
      const response = await request(app).get(`/workers/job`).set(`Authorization`, `Bearer ${tokenClient}`);
...
describe(`DELETE /clients/jobs/:jobId`, () => {
  ...
  describe(`Failed`, () => {
    ...
    test(`Failed 403, Attempting to cancel other client job order`, async () => {
      ...
      const response = await request(app).delete(`/clients/jobs/${jobIdError}`).set(`Authorization`, `Bearer ${tokenClientEmpty}`);
...
describe(`PATCH /workers/jobs/:jobId/worker`, () => {
  ...
  describe(`Failed`, () => {
    ...
    test(`Failed 403, Other Worker Attempt to confirm a job`, async () => {
      const jobId = String(jobSeed[2]._id);
      const response = await request(app).patch(`/workers/jobs/${jobId}/worker`).attach(`image`, `./src/files/test1.jpg`).set(`Authorization`, `Bearer ${tokenWorkerEmpty}`);
...
describe(`PATCH /clients/jobs/:jobId/client`, () => {
  ...
  describe(`Failed`, () => {
    ...
    test(`Failed 401, Unauthenticated Invalid Token`, async () => {
      const jobId = String(jobSeed[2]._id);
      const response = await request(app).patch(`/clients/jobs/${jobId}/client`).set(`Authorization`, `Bearer ${tokenClient}fwfbda`);
    ...
    test(`Failed 404, Job Not Found`, async () => {
      const jobIdError = `66dea5d32974b4cd98e9c333`;
      const response = await request(app).patch(`/clients/jobs/${jobIdError}/client`).set(`Authorization`, `Bearer ${tokenClient}`);
    ...
    test(`Failed 403, Attempting to confirm other client job order`, async () => {
      const jobId = String(jobSeed[2]._id);
      const response = await request(app).patch(`/clients/jobs/${jobId}/client`).set(`Authorization`, `Bearer ${tokenClientEmpty}`);
